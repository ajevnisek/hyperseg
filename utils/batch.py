...
parser.add_argument('-cf', '--corresponding_files', action='store_true',
                    help='assume the specified source and target files are corresponding pairs')


def main(source, target=None, output=None, func_obj=None, postfix=None, out_postfix=None, indices=None,
         skip_existing=False, reverse_output=False, ignore_output=False, corresponding_files=False):
    ...
    elif corresponding_files:
        n = min(len(source_paths), len(target_paths))
        input_paths = list(zip(source_paths[:n], target_paths[:n]))
    else:
    ...
    for i, curr_input in enumerate(input_paths):
        if isinstance(curr_input, (list, tuple)):
            ...
            out_vid_name = os.path.splitext(os.path.basename(curr_input))[0] + out_postfix
            curr_input = [curr_input]
        out_vid_path = os.path.join(output, out_vid_name) if output is not None else None
        if skip_existing and out_vid_path is not None and os.path.exists(out_vid_path):
    ...
    while i < len(inputs):
        if os.path.isfile(inputs[i]):
            ...
            if (i + 1) < len(inputs) and os.path.splitext(inputs[i + 1])[1] == '.txt':
                ...
                file_rel_paths = [p.split() for p in file_rel_paths]
                file_abs_paths = [[os.path.join(inputs[i], p) for p in pair] for pair in file_rel_paths]
                file_abs_paths = [p[0] if len(p) == 1 else p for p in file_abs_paths]
                input_paths += file_abs_paths
