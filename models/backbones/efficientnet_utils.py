...
GlobalParams = collections.namedtuple('GlobalParams', [
    ...
    'drop_connect_rate', 'depth_divisor', 'min_depth', 'out_feat_scale', 'base_embedding_channels', 'head', 'pool',
    'return_features'])
...
def efficientnet(width_coefficient=None, depth_coefficient=None, image_size=None, dropout_rate=0.2,
                 drop_connect_rate=0.2, num_classes=1000, out_feat_scale=0.25, blocks_args=None,
                 base_embedding_channels=1280, head=nn.Linear, pool=False, return_features=False):
...
def load_pretrained_weights(model, model_name, weights_path=None, load_fc=True, advprop=False):
    ...
    """
    print('Loading pretrained weights for {}...'.format(model_name))
    if model_name[-2] == 'c' or model_name[-2] == 's':
        model_name = f'efficientnet-b{model_name[-1]}'
    ...
    if load_fc:
        ...
        pretrained_dict = {k: v for k, v in state_dict.items() if
                           (k in model_state_dict) and (model_state_dict[k].shape == state_dict[k].shape)}
        ret = model.load_state_dict(pretrained_dict, strict=False)
